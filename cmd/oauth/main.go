...
func run() error {
	...
	if err := writeEnvFile(*envOut, envValues{
		ClientID:     *clientID,
...
type envValues struct {
	ClientID, ClientSecret, APIKey, VIN, RefreshToken string
}

func writeEnvFile(path string, v envValues) error {
	var b strings.Builder
	fmt.Fprintf(&b, "# Generated by volvo-charge-monitor/oauth at %s\n", time.Now().UTC().Format(time.RFC3339))
	fmt.Fprintf(&b, "VOLVO_CLIENT_ID=%s\n", v.ClientID)
	fmt.Fprintf(&b, "VOLVO_CLIENT_SECRET=%s\n", v.ClientSecret)
	fmt.Fprintf(&b, "VOLVO_API_KEY=%s\n", v.APIKey)
	fmt.Fprintf(&b, "VOLVO_VIN=%s\n", v.VIN)
	fmt.Fprintf(&b, "VOLVO_REFRESH_TOKEN=%s\n", v.RefreshToken)
	fmt.Fprintf(&b, "POLL_INTERVAL=3h\n")
	fmt.Fprintf(&b, "TOKEN_STORE_PATH=/data/token.json\n")
	fmt.Fprintf(&b, "HTTP_ADDR=:8080\n")
